/// @desc Revenge can only work for the pokemon.
...
onDamage = function (_dmg, _damaged_player_id, _action, _pokemon_id_attack, _state_id) {
	if _damaged_player_id != pokemon_id
	if _action[? "dmg_element"] > 0 {
		var _coef = _action[? "dmg_material"] / _action[? "dmg_element"]
		_action[? "damage"] = (_pokemon_id_attack.health_max - _pokemon_id_attack.health_cur)
		_action[? "dmg_material"] = _action[? "damage"] * _coef
